...
def filter(uf_value_storfs: list) -> list:
    ...
    Maximise Sum(all identified StORFs)
    S.t:
        For each StORF, s, belonging to a group of overlapping StORFs, S,:
            0 <= sum(s.value) <= max([s.value for s in S])
    with bounds:
...
def main():
    ...
    # !TODO output filtered StORFs to fasta file
