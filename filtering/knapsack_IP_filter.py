...
def ip_set_obj(prob: LpProblem, storf_groups: list, ip_vars: list) -> None:
    ...
    for storf_group in storf_groups:
        for storf in storf_group:
            # create StORF IP variable representation using StORF ID (variable) and value (coefficient)
            obj_expression.append((ip_vars[ip_var_id], 1))
            ip_var_id += 1
...
def non_ip_filter(storf_list) -> list:
    """
    Currently includes filtering by stop codon type, and average gc profile
    """
    # TODO convert these to IP constraints
        ...
        if FILTER_ARGS.stop_codon_type:  # filter according to allowed stop codon types
            ...
            for sct in FILTER_ARGS.stop_codon_type:
                if sct == stop_codon_1 or sct == stop_codon_2:
                    remove = False
            if remove:  # remove any StORF with disallowed stop codons
                del storf_list[index]
    return storf_list
...
def ip_filter(uf_value_storfs: list) -> list:
    ...
    to maximise the number of StORFs selected
    from the initial list, given the 
    ...
    prob = LpProblem("StORF_IP_Filter", LpMaximize)
...
def read_fasta() -> list:
    unfiltered_storfs = []
    with open(FILTER_ARGS.input_file) as storf_file:
...
def init_cl_args() -> argparse.ArgumentParser:
    ...
    """
    # TODO add argument exceptions/general error handling
    args = argparse.ArgumentParser(description='StORF filter options')
    args.add_argument('-f', '--input_file', dest="input_file", type=str, 
                      help="input fasta file destination")
    ...
    args.add_argument('-gc', '--gc_profile', dest='gc_profile', nargs=2,
                      ...
                      help='VAR: acceptable percentage range ' \
    ...
    # TODO e.g. with average=mode, only select StORFs with least used stop codons
    ...
    if not FILTER_ARGS.input_file:
        args.error("No fasta input StORF file provided (-f)")
    if not (FILTER_ARGS.overlap_range or FILTER_ARGS.storf_range or FILTER_ARGS.gc_profile 
            or FILTER_ARGS.stop_codon_type or FILTER_ARGS.stop_codon_av):
        args.error("No filtering options provided")


def main():
    # TODO general refactor+documentation
    init_cl_args()
    uf_storfs = read_fasta()
    ip_filtered_storfs = ip_filter(uf_storfs)
